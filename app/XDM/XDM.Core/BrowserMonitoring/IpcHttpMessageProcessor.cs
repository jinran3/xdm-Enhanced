...
namespace XDM.Core.BrowserMonitoring
{
    public class IpcHttpMessageProcessor
    {
        ...
        private static readonly YouTubeDownloadService Youtube = new YouTubeDownloadService();
        ...
        private void OnYoutubeMessage(RequestContext context)
        {
            AddYoutubeCorsHeaders(context);
            // CORS preflight (OPTIONS) carries no body -> answer with CORS headers only.
            ...
            var request = JsonConvert.DeserializeObject<YouTubeDownloadService.Request>(Encoding.UTF8.GetString(context.RequestBody!));
            if (request == null)
            {
                return;
            }
            ...
            var service = Youtube;
            YouTubeDownloadService.Result result;

            if ("download".Equals(request.Action, StringComparison.OrdinalIgnoreCase))
            {
                if (!int.TryParse(request.Res, out var height))
                {
                    height = 2160;
                }
                result = service.Download(request.Url ?? "", height, request.Browser, request.Format, request.Lang);
            }
            else if ("download-list".Equals(request.Action, StringComparison.OrdinalIgnoreCase))
            {
                result = service.DownloadList(request.Url ?? "", request.Browser, request.Format);
            }
            else if ("download-subs".Equals(request.Action, StringComparison.OrdinalIgnoreCase))
            {
                result = service.DownloadSubs(request.Url ?? "", request.Browser, request.Lang, request.Ext);
            }
            else
            {
                result = service.List(request.Url ?? "", request.Browser);
            }
        ...
        private static void AddYoutubeCorsHeaders(RequestContext context)
        {
            // The /youtube endpoint is also callable from local pages (e.g. the
            // test harness), so answer cross-origin preflights on localhost.
            context.AddResponseHeader("Access-Control-Allow-Origin", "*");
            context.AddResponseHeader("Access-Control-Allow-Methods", "POST, OPTIONS");
            context.AddResponseHeader("Access-Control-Allow-Headers", "Content-Type");
        }
